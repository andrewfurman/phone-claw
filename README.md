...
| `himalaya_email_image_inspect` | `/cli/himalaya/email-image-inspect` | EC2 bridge via Himalaya + Vercel AI Gateway | Read-only vision/OCR for one envelope's images. Reuses Himalaya image extraction, calls `PHONECLAW_VISION_MODEL` (default `google/gemini-3.8-flash`) through AI Gateway, and returns description, OCR text, and spoken `answer_text`. Requires `AI_GATEWAY_API_KEY` on the bridge. |
