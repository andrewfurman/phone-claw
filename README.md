...
- `.env.example` - the public environment template, placeholders only (copy it to `.env`).
...
| `image_inspect` | `/cli/image/inspect` | EC2 bridge via Himalaya and/or public HTTPS fetch + Vercel AI Gateway | Generic read-only vision/OCR. Exactly one source: email `id`/`envelope_id` (same as email-image-inspect) or public https `url` (private/localhost blocked; image/* size/content-type capped). Calls `PHONECLAW_VISION_MODEL` through AI Gateway. Requires `AI_GATEWAY_API_KEY` on the bridge. |
| `himalaya_email_image_inspect` | `/cli/himalaya/email-image-inspect` | EC2 bridge via Himalaya + Vercel AI Gateway | Compatibility alias for email-source `image_inspect`. Reuses Himalaya image extraction and the same AI Gateway vision path. |
