...
- Cap command output and timeouts. Specialized tools use fixed `execFile` invocations. The additive `run_cli` / `POST /cli/run` path runs allow-listed working directories through `/bin/bash -lc` with timeouts, truncated/redacted output, secret-path blocks, and confirmation gates for destructive commands.
