...
- Keep CLI tools read-only by default. Any write tool must be narrowly scoped and confirmation-gated. Email archive/draft tools cannot send mail; emergency sends are isolated in `himalaya_email_send` and require `emergency=true`, an exact verbal preview, and a second confirmation. Claude Code task submission is also confirmation-gated and runs asynchronously.
