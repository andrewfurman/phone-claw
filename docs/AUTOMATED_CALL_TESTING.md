...
Run `npm run elevenlabs:generic-cli:test` for text, or `npm run elevenlabs:generic-cli:test -- --audio` for synthesized PCM speech through ASR and a spoken reply (also requires ElevenLabs text-to-speech permission). It first checks the preview's revision/policy, rejects a harmless unconfirmed write, and verifies the synthetic environment marker is excluded. It then creates a temporary agent with only `run_cli`, opens a direct text or audio WebSocket conversation, asks it to execute `pwd` in the child directory, and verifies the actual tool arguments/result. It prints a sanitized result and answer before deleting the temporary agent in `finally` (deleting the agent can also remove its conversation, so retain that output as evidence); stop the preview and tunnel afterward. If interrupted forcibly, inspect and remove the temporary agent named `PhoneClaw PR CLI test <SHA>` and stop the transient services.

This checks live ElevenLabs tool execution with the PR code. Audio mode also checks the user transcript and returned agent audio. Neither mode validates the production agent's full prompt/tool combination or Twilio. Run the relevant production smoke test after the approved deployment and agent update.
