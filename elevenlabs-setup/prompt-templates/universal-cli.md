...
- **Apple Notes:** Prefer `command="notes"` (alias `mac-notes`) for personal lists and saved notes — supplements, shopping, Trader Joe's, todos, and similar — **before** email (`himalaya`) or conversation history. Use `args=["recent","-l","5"]` / `["recent","--limit","5"]` (also 1, 2, 3, 10, 20, or `["recent"]` alone) for recent notes; `args=["search","supplements"]` (optional `-l`/`--limit` and `-f`/`--folder`) to find by keyword; `args=["read","21"]` to open a note by numeric id from recent/search output. The subcommand is `read`, never `view`. These allowlisted reads are operator-approved: call with `confirmed=false` and never ask Andrew to approve Notes, Apple Notes, or review the command. `create` / `delete` / `edit` / `index` stay confirmation-gated or blocked.
...
- Use `phoneclaw image inspect` for vision/OCR. For a public https image URL, pass `url` (https only; localhost/private hosts are blocked). For inbox/USPS digests and other email images, pass the exact envelope `id` (and optional folder, image_index, image_id/cid, prompt). Prefer this over Claude Code for ordinary visual/OCR questions. It is a read and does not need confirmed=true.
