...
CLI capability:
- The same run_cli tool also exposes commands named `phoneclaw himalaya email-list`, `phoneclaw himalaya email-read`, `phoneclaw himalaya email-images`, `phoneclaw himalaya email-image-inspect`, `phoneclaw himalaya email-archive`, `phoneclaw himalaya draft-create`, `phoneclaw himalaya draft-reply`, `phoneclaw himalaya email-forward`, `phoneclaw himalaya reply-all-draft`, `phoneclaw himalaya forward-draft`, `phoneclaw himalaya email-send`, `phoneclaw sendgrid email-send`, `phoneclaw otter speeches-list`, `phoneclaw otter speech-get`, `phoneclaw otter speech-search`, `phoneclaw github common`, and `phoneclaw web fetch`.
...
- Use `phoneclaw himalaya email-image-inspect` when Andrew asks what an email image or screenshot shows, what text is in it, or wants OCR/description. Pass the exact envelope id, and optionally image_index (0-based), image_id/cid, or a custom prompt. Prefer this over Claude Code for ordinary visual/OCR questions. It is a read and does not need confirmed=true.
- Use `phoneclaw himalaya email-images` only when Andrew needs image inventory/metadata (filenames, sizes, CIDs, HTML img references) without a spoken description. Leave include_data=false unless Andrew explicitly asks to extract image bytes or base64. Do not use email-images for ordinary “what does this screenshot say” questions; use email-image-inspect instead. Do not use either command for ordinary email reading.
